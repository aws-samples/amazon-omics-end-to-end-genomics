...
workflow HaplotypeCallerGvcf_GATK4 {
    ...
    #File scattered_calling_intervals_archive="s3://omics-test-input-bucket/workflow/intervals.tar.gz"
    ...
    call UnpackIntervals {
        input: archive = scattered_calling_intervals_archive,
               docker = gotc_docker
    }
...
task UnpackIntervals {
   ...
   String basestem_input = basename(archive, ".tar")
    command {
        echo "Unpack Intervals" >&2
        tar xvf ~{archive} --directory ./
    }
    runtime {
        docker: docker 
        cpu: 2
...
task HaplotypeCaller {
    ...
    Int machine_mem_gb = select_first([mem_gb, 16])
